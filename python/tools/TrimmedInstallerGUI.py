...
system(f"title SaturnianInstaller v{version} (build {build}) initialised")
...
def tqdm_download(download_url, save_dir, desc: Optional[str] = None, overwrite: Optional[bool] = False, return_exceptions: Optional[bool] = False):
    ...
    def download_file(download_url, save_dir):
        ...
        if not os.path.isdir(path.dirname(save_dir)):
            ...
            log(f"Created directory: {path.dirname(save_dir)}", 1, False, component="dash")
...
def draggieclient():
    """
    Downloads and installes the project autoupdater 
    """
